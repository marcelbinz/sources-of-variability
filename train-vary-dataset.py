...
def run(
    ...
    df, dict_info = ut.load_dataset(dataset_name)
    ...
    tf = "log_values"  # "z" or "mean_center_only" or "log_values"
    df = ut.zscore_pairs(df, dict_info["l_colnames_grouped_zscale"], tf=tf)

    # shuffle trial ids and participant ids to create dfs with theoretical upper bounds on learning

    # n_trials needs to be fixed in make_conditions
    # can just count number of trials per participant and use that value

    df_original, df_id_nohist, df_shared_hist, df_shared_nohist = ut.make_conditions(
        df, n_trials, dict_info["n_participants_total"])
    ...
    run = wandb.init(
        ...
        name=f"condition={condition_name}_moreshuffle={more_shuffle}_tf={tf}_dmodel={d_model}_dff={d_ff}_numlayers={num_layers}_ntrials_train={dict_info['n_trials_train']}_masktype={masktype}_windowsize={windowsize}",
        config={
